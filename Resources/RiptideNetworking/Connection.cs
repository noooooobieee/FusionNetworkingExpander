...
namespace Riptide
{
    ...
    public abstract class Connection
    {
        ...
        internal bool HandleWelcomeResponse(Message message)
        {
            ...
            if (Id != id)
                RiptideLogger.Log(Utils.LogType.Error, Peer.LogName, $"Client has assumed ID {id} instead of {Id}!");
            ...
            internal override bool ShouldHandle(ushort sequenceId)
            {
                ...
                if (sequenceGap != 0)
                {
                    ...
                    if (sequenceGap > 0)
                    {
                        // The received sequence ID is newer than the previous one
                        if (sequenceGap > 64)
                            RiptideLogger.Log(Utils.LogType.Warning, connection.Peer.LogName, $"The gap between received sequence IDs was very large ({sequenceGap})!");
