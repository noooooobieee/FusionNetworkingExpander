...
namespace Riptide
{
    ...
    public class Client : Peer
    {
        ...
        public bool Connect(string hostAddress, int maxConnectionAttempts = 5, byte messageHandlerGroupId = 0, Message message = null, bool useMessageHandlers = true)
        {
            ...
            {
                RiptideLogger.Log(Utils.LogType.Error, LogName, connectError);
            ...
            if (message != null)
            {
                if (message.ReadBits != 0)
                    RiptideLogger.Log(Utils.LogType.Error, LogName, $"Use the parameterless 'Message.Create()' overload when setting connection attempt data!");
            ...
            StartTime();
            Heartbeat();
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Connecting to {connection}...");
            return true;
        }
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection)
        {
            switch (header)
            {
                ...
                case MessageHeader.ClientDisconnected:
                    ...
                    break;
                default:
                    RiptideLogger.Log(Utils.LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.BytesInUse} bytes.");
                    break;
            }
        ...
        protected virtual void OnConnected()
        {
            ...
            connectMessage = null;
            RiptideLogger.Log(Utils.LogType.Info, LogName, "Connected successfully!");
        ...
        protected virtual void OnConnectionFailed(RejectReason reason, Message message = null)
        {
            connectMessage.Release();
            connectMessage = null;
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Connection to server failed: {Helper.GetReasonString(reason)}.");
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Disconnected from server: {Helper.GetReasonString(reason)}.");
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Client {clientId} connected.");
        ...
        {
            RiptideLogger.Log(Utils.LogType.Info, LogName, $"Client {clientId} disconnected.");
